...
/*   Updated: 2018/07/02 15:26:11 by mschempe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// if you add [1] to the static var ("str") and [fd] to all instances of "str" afterwards, it works with multiple
// file descriptors. Code as follows:
// static char *str[1];
// char			buff[BUFF_SIZE + 1];
// and then:
// if ((!str[fd] && (str[fd] = ft_strnew(sizeof(*str[fd]))) == NULL) || !line || fd < 0 || read(fd, buff, 0) < 0)
// and then [fd] at each instance of "str"
